...
<body>
    ...
    <div class="container">
        ...
        <!-- Form Edit Artikel -->
        <form method="POST" action="{{ route('admin.artikel.update', $article['id']) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="title">Nama Teh</label>
                <input type="text" id="title" name="nama" value="{{ $article['nama'] }}" placeholder="Masukkan nama teh" required>
            </div>
            <div class="form-group">
                <label for="content">Deskripsi Artikel</label>
                <textarea id="content" name="deskripsi" placeholder="Deskripsikan artikel" required>{{ $article['deskripsi'] }}</textarea>
            </div>
            <div class="form-group">
                <label for="lokasi">Lokasi</label>
                <input type="text" id="lokasi" name="lokasi" value="{{ $article['lokasi'] }}" placeholder="Masukkan lokasi" required>
            </div>
            <div class="form-group">
                ...
                @if($article['gambar'])
                    <br>
                    <img src="{{ $article['gambar'] }}" alt="{{ $article['nama'] }}" style="max-width: 150px; margin-top: 10px;">
                @endif
            </div>
